...
export const createMaps = async (glassCount: number, iterations: number) => {
  let colors = [
    "red",
    "blue",
    "green",
  ...
  if (glassCount < 5) return;

  let levels: { [keys: string]: string[][][] } = {};

  colors = colors.slice(0, glassCount);

  let raw = await fsP.readFile(path.join(__dirname, "levels.json"), {
    encoding: "utf8",
  });
  let file: { [keys: string]: string[][][] } = await JSON.parse(raw);

  for (let i = 0; i < iterations; i++) {
    let glasses = setGlasses(colors);

    const initialState = JSON.stringify(glasses);
    let lastState = solver(glasses);

    if (isGameCompleted(lastState)) {
      if (typeof levels[colors.length] === "undefined")
        levels[colors.length] = [];

      if (file[colors.length] && file[colors.length].includes(glasses)) {
        console.log("\nGame already in found\n");
        continue;
      }

      levels[colors.length].push(JSON.parse(initialState));

      // console.log(levels[colors.length].length);
    }
  }

  ///

  // let filteredLevels = levels[colors.length].filter(
  //   (game) => !file[colors.length].includes(game)
  // );
  if (typeof file[colors.length] === "undefined") file[colors.length] = [];
  file[colors.length].push(...levels[colors.length]);
  ...
  for (const item in file) {
    console.log(`${item} has ${file[item].length}\n`);
  }
};
